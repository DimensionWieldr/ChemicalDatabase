...
public class ChemicalDatabase {
    ...
    public boolean editElements(Element toBeEdited, String commonName, double stpDensity, double bp, double mp, String colour, String molecularFormula, int numProton, double electronegativity, int numNeutron, double molarMass, double ionizationEnergy){
        //Save temporary variables from toBeEdited element before removal
        List<Reaction> oldReactionsInvolved = toBeEdited.getReactionsInvolved();
        ...
        this.removeChemical(toBeEdited.getCommonName());
        ...
        if(!(this.numProtonRepeats(numProton) || this.commonNameRepeats(commonName) || this.elementMolecularFormulaRepeats(molecularFormula))){
            toBeEdited = new Element(commonName, stpDensity, molarMass, bp, mp, colour, numProton, numNeutron, ionizationEnergy, electronegativity, molecularFormula);
            successful = true;
        }
        
        //Add back chemical
        this.chemicals.add(toBeEdited);
        this.updateTable();
        
        //Re-simulate reactions
        Reaction newReaction;
        for (int i = 0 ; i < length ; i++) {
            if (toBeEdited.getCommonName().equals(reactant1Names[i]) && toBeEdited.getCommonName().equals(reactant2Names[i])) {
                newReaction = this.simulateReaction(toBeEdited, toBeEdited);
                newReaction.getProduct().setCommonName(productNames[i]);
            } else if (toBeEdited.getCommonName().equals(reactant1Names[i])) {
                newReaction = this.simulateReaction(toBeEdited, reactant2Array[i]);
                newReaction.getProduct().setCommonName(productNames[i]);
            } else if (toBeEdited.getCommonName().equals(reactant2Names[i])) {
                newReaction = this.simulateReaction(reactant1Array[i], toBeEdited);
                newReaction.getProduct().setCommonName(productNames[i]);
            }   
        }
    ...
    public boolean editBinaryCompounds(BinaryCompound toBeEdited, String commonName, double stpDensity, double bp, double mp, String colour){
        this.chemicals.remove(toBeEdited);
        toBeEdited.getComponent1().getComponent().getCompoundsInvolved().remove(toBeEdited);
        toBeEdited.getComponent2().getComponent().getCompoundsInvolved().remove(toBeEdited);
        ...
        toBeEdited = new BinaryCompound(commonName, stpDensity, toBeEdited.getMolarMass(), bp, mp, colour, toBeEdited.getCharge(), toBeEdited.getComponent1(), toBeEdited.getComponent2());
        toBeEdited.setReactionsInvolved(oldReactionsInvolved);
        
        for (int i = 0 ; i < toBeEdited.getReactionsInvolved().size() ; i++) {
            toBeEdited.getReactionsInvolved().get(i).setProduct(toBeEdited);
        }
