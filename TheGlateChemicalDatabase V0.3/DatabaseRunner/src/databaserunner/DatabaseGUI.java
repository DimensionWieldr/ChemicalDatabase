...
public class DatabaseGUI {
    ...
    public static Reaction simulateReaction(int id, Element reactant1, Element reactant2){
        Reaction newReaction = new Reaction(id, reactant1, reactant2);
        if (reactant1.getOxidationNum() != 0 && reactant2.getOxidationNum() != 0 && database.isNewReaction(newReaction)) {
            database.addReaction(newReaction);
            database.addBinaryCompound(newReaction.getProduct());
            return newReaction;
        }
        return null;
    }
