...
public class DatabaseGUI {
    ...
    public static final String[] ON_CLOSE_OPTIONS = {"Yes", "No", "Cancel"};
    ...
    public char validateFile(String fileName) {
        ...
        for(int i = 0; i < tempFileName.length; i++) {
            ...
            for (int j = 0 ; j < ILLEGAL_CHARS.length && !containsIllegalChar ; j++) {
                if (tempFileName[i] == ILLEGAL_CHARS[j])
    ...
    public int savePrompt(boolean close) {
        System.out.println(madeChanges);
        if (madeChanges) {
            int optionNum = JOptionPane.showOptionDialog(null, "Do you want to save?", "", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE, null, DatabaseGUI.ON_CLOSE_OPTIONS, null);
            
            if(optionNum == 0) 
                saveDialog();
            if(close && optionNum != 2)
                System.exit(0);
    ...
    public boolean removeChemical(String name){
        madeChanges = true;
        return this.database.removeChemical(this.searchChemicalsByName(name));
    }
    public void removeReaction(int index){
        madeChanges = true;
        this.database.removeReaction(this.searchReactionsByID(index));
